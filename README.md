# Hi, I'm Enes 👋

I'm an AI engineer focused on building reliable, measurable, and useful AI systems.

My current interests include:

- AI coding agents and developer tooling
- LLM evaluation, retrieval, and context engineering
- NLP and machine learning for Turkish
- AI safety and secure agent workflows

## Selected work

### [Turkish POS Tagger](https://github.com/Enesevki/POSTagger)

A CRF-based Turkish part-of-speech tagging pipeline with data preparation,
cross-validation, learning curves, error analysis, and an interactive Streamlit demo.

[Try the live demo](https://turkishpostagger.streamlit.app) ·
[Watch the project walkthrough](https://www.youtube.com/watch?v=AQlYbdYh_bM)

## Currently building

### RepoGuard

An open-source security layer that inspects untrusted repositories before AI coding
agents and developer tools read instructions, install dependencies, or execute code.

The goal is simple: make repository trust explicit, explainable, and automatable.

## What I care about

I enjoy projects where model quality can be measured, failures can be explained,
and the final tool is practical enough to use in a real development workflow.

I'm open to collaborating on AI infrastructure, evaluation, NLP, and developer tools.
